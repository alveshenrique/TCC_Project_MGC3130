...
int main()
{
    ...
    if(gestic_query_fw_version(gestic, version, sizeof(version), 100) == 0) {
        /* The version should already include a terminating \0 but
        ...
        printf("Could not read running firmware library version.\n"
               "This probably means that there is no library present.\n\n"
               "[ GestIC Library ]\n\n"
               "To be flashed:\n%s\n\n"
               "[ GestIC Library Loader ]\n\n"
               "To be flashed:\n%s\n\n",
               Library.fw_version, Loader.fw_version);
    }

    printf("Do you really want to flash the new images (yes,no)? ");
    fflush(stdout);
    fgets(confirm, sizeof(confirm), stdin);
    if(strcasecmp(confirm, "yes\n") != 0 && strcasecmp(confirm, "y\n") != 0) {
        gestic_cleanup(gestic);
        gestic_free(gestic);
        return 0;
    }
